...
//with coroutines
    ...
    {
        auto self(shared_from_this());
        const auto[ec, length] = co_await async_read_some(m_socket, boost::asio::buffer(m_data, m_data_max_length));
    ...
    auto async_write_some(SyncReadStream &s, DynamicBuffer &&buffers)
    {
        struct Awaiter
        {
            ...
            void await_suspend(std::coroutine_handle<> coro)
            {
                boost::asio::async_write(s, std::move(buffers), [this, coro](auto ec, auto sz) mutable
                {
                    this->ec = ec;
                    this->sz = sz;
                    coro.resume();
    ...
    {
        auto self(shared_from_this());
        const auto[ec, length] = co_await async_write_some(m_socket, boost::asio::buffer(m_data, m_data_max_length));
    ...
    auto do_write(std::size_t length)
    {
        ...
        struct Awaiter
        {
            ...
            void await_suspend(std::coroutine_handle<> coro)
            {
                const auto[ec, sz] = co_await async_write(ssn->m_socket, boost::asio::buffer(ssn->m_data, length));
                this->ec = ec;
                coro.resume();
            }
        };
...
//        while(true)
//        {
//            const auto[ec, sz] = co_await async_read_some(m_socket, boost::asio::buffer(m_data, m_data_max_length));
//            if(!ec)
//            {
...
//    }
    ...
    tcp::socket m_socket;
    static constexpr std::size_t m_data_max_length{1024};
    char m_data[m_data_max_length];
};
